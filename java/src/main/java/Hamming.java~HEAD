...
/**
 * Created by jorge on 28/03/17.
 */
public class Hamming {

    public static Long MAX_EXP = 100l;
    private static TreeSet<Long> hammingNumbers;

    public static long hamming(int n) {
        if(hammingNumbers==null){
            hammingNumbers = calculateHammingNumbers();
        }

        Iterator<Long> it = hammingNumbers.iterator();
        int i = 0;
        while(it.hasNext()) {
            Long result = it.next();
            if(i==n-1){
    ...
    private static TreeSet<Long> calculateHammingNumbers() {
        TreeSet<Long> hammingNumbers = new TreeSet<>();
        for(long i = 0; i< MAX_EXP ; i++){
            for(long j = 0; j < MAX_EXP; j++){
                for(long k = 0; k < MAX_EXP; k++){
                    hammingNumbers.add(hamming(i,j,k));
                }
            }
        }
    ...
    public static long hamming(long i, long j, long k) {
        Double pow =  Math.pow(2,i)*Math.pow(3,j)*Math.pow(5,k);
